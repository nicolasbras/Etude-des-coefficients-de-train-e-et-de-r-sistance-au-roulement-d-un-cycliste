...
import numpy as np
txt_file_path = r"D:\Ecole\prépa\psi\Mesures_pneus_pression_tipe\5_bars.txt"
...
mode_path = r"D:\Ecole\prépa\psi\Mesures_pneus_pression_tipe\5_bars.txt"
...
del LON[:19]
del ELE[:19]
del TEMPS[:20]#le premier temps apparait 2 fois dans le fichier
...
emax=0
for t in range(0,len(DTOT)):
    if emax<abs(position(TEMPS_TOT[t])-DTOT[t]):
        emax=abs(position(TEMPS_TOT[t])-DTOT[t])
print(emax)
dposition=
dACC=dposition
AE=np.zeros(10)
BE=np.zeros(10)
for i in range(10):
    acc=rd.normal(ACC,dACC)
    print(acc)
    Poly=np.polyfit(VQ,acc*m,1)
    AE[i]=Poly[0]
    BE[i]=Poly[1]
print(AE)
print(BE)
