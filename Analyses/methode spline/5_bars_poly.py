...
del LON[:25]
del ELE[:25]
del TEMPS[:26]#le premier temps apparait 2 fois dans le fichier
...
L=np.polyfit(TEMPS_TOT,DTOT,5)
t=np.linspace(0,TEMPS_TOT[-1],len(TEMPS_TOT))


def position(t):
    return L[0]*t**5+L[1]*t**4+L[2]*t**3+L[3]*t**2+L[4]*t+L[5]
...
def vitesse(t):
    return (5*L[0]*t**4+4*L[1]*t**3+3*L[2]*t**2+2*L[3]*t+L[4])#0.32

plt.plot(t,vitesse(t),'b+')
plt.show()

def acceleration(t):
    return (20*L[0]*t**3+12*L[0]*t**2+6*L[1]*t+2*L[2])
